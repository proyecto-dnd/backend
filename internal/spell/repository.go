...
func (r *spellMySqlRepository) GetByCharacterDataId(characterId int) ([]domain.Spell, error) {
	...
	for rows.Next() {
		var spell domain.Spell
		if err := rows.Scan(&spell.SpellId, &spell.Name, &spell.Description, &spell.Range, &spell.Ritual, &spell.Duration, &spell.Concentration, &spell.CastingTime, &spell.Level, &spell.DamageType, &spell.DifficultyClass, &spell.Aoe, &spell.School); err != nil {
			return nil, err
		}
		spells = append(spells, spell)
	}
