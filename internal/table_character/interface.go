...
type RepositoryTableCharacter interface {
	...
	GetByUserId(userid string)([]domain.TableCharacter, error)
	GetByUserIdAndCampaignId(userid string, campaignid int)([]domain.TableCharacter, error)
	GetByCampaignId(campaignid int)([]domain.TableCharacter, error)
	Update(character domain.TableCharacter) (domain.TableCharacter, error)
	Delete(id int)error
}